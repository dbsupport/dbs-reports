...
public class AbsenceExporter {
	...
	@Autowired
	public AbsenceExporter(SqlExecutor<Map<String, String>> executor, AbsenceValidatorProcessor validatorProcessor, AbsenceInputSplitter splitter) {
		this.executor = executor;
		this.validatorProcessor = validatorProcessor;
		this.splitter = splitter;
	}

	public AbsenceResult export(final List<AbsenceInput> inputs) {
	...
	private List<AbsenceOutput> export(AbsenceInput input) {
		...
		for (AbsenceInput.AbsenceRange range : splitter.split(input)) {
			...
			AbsenceOutput absence = AbsenceOutputBuilder.builder()
					...
					.dateTo(range.getEnd())
					.date(input.getDateAsDate())
					.employmentDate(employmentDate)

					.hrdata(fetchHrData(input.getPesel(), input.getNip(), employmentDate))
					.sicknessCode(input.getAvailableSicknessCode())
