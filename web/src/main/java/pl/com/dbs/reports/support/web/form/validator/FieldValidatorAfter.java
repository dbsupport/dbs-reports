...
public class FieldValidatorAfter<T> extends AFieldValidator<T> {
	...
	public void validate(AField<T> field, LinkedList<AField<?>> fields) throws FieldValidatorException {
		if (field instanceof FieldDate) {
			...
			if (NOW.equalsIgnoreCase(this.parameter)) {
				when = new Date();
				if (f1.getValueTypized()==null||!when.before(f1.getValueTypized())) {
				...
				if (when==null||f1.getValueTypized()==null||!when.before(f1.getValueTypized())) {
