...
<div class="<tiles:getAsString name="class"/><c:choose><c:when test="${status.error}"> <tiles:getAsString name="errorclass"/></c:when></c:choose>">
	...
	<input name="${name}.value" value="<tiles:getAsString name="value"/>" type="text" class="<tiles:getAsString name="inputclass"/> input-datepicker" data-date-format="<tiles:getAsString name="format"/>" placeholder="<tiles:getAsString name="tooltip"/>" <tiles:getAsString name="attributes"/>/>
